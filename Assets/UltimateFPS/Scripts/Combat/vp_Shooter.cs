...
public class vp_Shooter : vp_Component
{
	...
	protected virtual void PlayFireSound()
	{
		...
		Audio.pitch = URnd.Range(SoundFirePitch.x, SoundFirePitch.y) * Time.timeScale;
	...
	protected virtual void SpawnProjectiles()
	{
		...
		for (int v = 0; v < ProjectileCount; v++)
		{
			if (ProjectilePrefab != null)
			{
				...
				p.transform.Rotate(0, 0, URnd.Range(0, 360));									// first, rotate up to 360 degrees around z for circular spread
				p.transform.Rotate(0, URnd.Range(-ProjectileSpread, ProjectileSpread), 0);		// then rotate around y with user defined deviation
			}
		}
	...
	protected virtual void EjectShell()
	{
		...
		{
			if (URnd.value > 0.5f)
				s.rigidbody.AddRelativeTorque(-URnd.rotation.eulerAngles * ShellEjectSpin);
			else
				s.rigidbody.AddRelativeTorque(URnd.rotation.eulerAngles * ShellEjectSpin);
		}
