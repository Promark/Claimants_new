	...
	void Start()
	{
		...
		if (Physics.Raycast(ray, out hit, Range, (IgnoreLocalPlayer ? vp_Layer.Mask.BulletBlockers : vp_Layer.Mask.IgnoreWalkThru)))
		{
			...
			if (hit.transform.lossyScale == Vector3.one)								// if hit object has normal scale
				t.Rotate(Vector3.forward, URnd.Range(0, 360), Space.Self);	// spin randomly
			else
			{
			...
			{
				m_Audio.pitch = URnd.Range(SoundImpactPitch.x, SoundImpactPitch.y) * Time.timeScale;
				m_Audio.PlayOneShot(m_ImpactSounds[(int)URnd.Range(0, (m_ImpactSounds.Count))]);
			}
