...
public class vp_FPWeaponMeleeAttack : vp_Component
{
	...
	private void UpdateAttack()
	{
		...
		vp_Timer.In(SwingDelay, delegate()
		{
			...
			{
				Audio.pitch = URnd.Range(SoundSwingPitch.x, SoundSwingPitch.y) * Time.timeScale;
				Audio.clip = (AudioClip)SoundSwing[(int)URnd.Range(0, (SoundSwing.Count))];
				Audio.Play();
			}
	...
	void PickAttack()
	{
		...
		reroll:
			...
			attack = URnd.Range(0, States.Count - 1);
			if ((States.Count > 1) && (attack == m_AttackCurrent) && (URnd.value < 0.5f))
				goto reroll;
	...
	void SpawnImpactFX(RaycastHit hit)
	{
		...
		{
			Audio.pitch = URnd.Range(SoundImpactPitch.x, SoundImpactPitch.y) * Time.timeScale;
			Audio.PlayOneShot((AudioClip)SoundImpact[(int)URnd.Range(0, (SoundImpact.Count))]);
		}
