	...
	void Awake()
	{
		...
		foreach (Collider col in GetComponentsInChildren<Collider>())
		{
			if (col.rigidbody)
			{
				...
				Collider c = col;
				vp_Timer.In(URnd.Range(LifeTime * 0.5f, LifeTime * 0.95f), delegate()
				{
					if (c != null)
		...
		if (Sounds.Count > 0)
		{
			...
			m_Audio.clip = Sounds[(int)URnd.Range(0, (Sounds.Count))];
			m_Audio.pitch = URnd.Range(SoundMinPitch, SoundMaxPitch) * Time.timeScale;
			m_Audio.Play();
		}
