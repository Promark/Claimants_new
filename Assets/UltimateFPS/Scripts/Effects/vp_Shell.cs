...
public class vp_Shell : MonoBehaviour
{
	...
	void OnCollisionEnter(Collision collision)
	{
		...
		if (collision.relativeVelocity.magnitude > 2)
		{
			...
			if (URnd.value > 0.5f)
				m_Rigidbody.AddRelativeTorque(-URnd.rotation.eulerAngles * 0.15f);
			else
				m_Rigidbody.AddRelativeTorque(URnd.rotation.eulerAngles * 0.15f);
			...
			if (m_Audio != null && m_BounceSounds.Count > 0)
			{
				...
				m_Audio.PlayOneShot(m_BounceSounds[(int)URnd.Range(0, (m_BounceSounds.Count))]);
			}

		}
		// soft collision = time to determine if this shell lives or dies
		else if (URnd.value > m_Persistence)
		{
