...
public class NoiseEffect : MonoBehaviour
{
	...
	void OnRenderImage (RenderTexture source, RenderTexture destination)
	{
		...
		{
			scratchTimeLeft = URnd.value * 2 / scratchFPS; // we have sanitized it earlier, won't be zero
			scratchX = URnd.value;
			scratchY = URnd.value;
		}
		...
		mat.SetVector("_ScratchOffsetScale", new Vector4(
			scratchX + URnd.value*scratchJitter,
			scratchY + URnd.value*scratchJitter,
		...
		mat.SetVector("_Intensity", new Vector4(
			URnd.Range(grainIntensityMin, grainIntensityMax),
			URnd.Range(scratchIntensityMin, scratchIntensityMax),
			0, 0 ));
